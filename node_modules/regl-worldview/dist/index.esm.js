...
import React__default, { Component, createRef, createElement, Fragment, forwardRef, useState, useCallback, useEffect, memo, useDebugValue, useContext } from 'react';
...
function Dimensions({
  ...
  const [dimensions, setDimensions] = useState(); // This resizeObserver should never change.

  const [resizeObserver] = useState(() => new ResizeObserverImpl(entries => {
...
class WorldviewContext {
  ...
  constructor({
    ...
    _defineProperty(this, "_needsPaint", false);
    ...
    _defineProperty(this, "contextAttributes", void 0);
...
class WorldviewBase extends Component {
  constructor(props) {
    ...
    _defineProperty(this, "_cameraListener", createRef());

    _defineProperty(this, "_tick", void 0);

    _defineProperty(this, "_dragStartPos", null);

    _defineProperty(this, "handleOffscreenMouseEvent", (e, mouseEventName) => {
    ...
    _defineProperty(this, "_onDoubleClick", (e, fromOffscreenTarget) => {
      this._onMouseInteraction(e, "onDoubleClick", fromOffscreenTarget);
    });

    _defineProperty(this, "_onMouseDown", (e, fromOffscreenTarget) => {
      this._dragStartPos = {
        x: e.clientX,
        y: e.clientY
      };
    ...
    _defineProperty(this, "_onMouseMove", (e, fromOffscreenTarget) => {
      this._onMouseInteraction(e, "onMouseMove", fromOffscreenTarget);
    });

    _defineProperty(this, "_onMouseUp", (e, fromOffscreenTarget) => {
    ...
    _defineProperty(this, "_onMouseInteraction", (e, mouseEventName, fromOffscreenTarget) => {
      const {
        worldviewContext
      } = this.state;
  ...
  render() {
    ...
    const canvasHtml = canvas ? null : createElement(Fragment, null, createElement("canvas", {
      style: {
        width,
        height,
        maxWidth: "100%",
...
const Worldview = forwardRef((props, ref) => createElement(Dimensions, null, ({
  width,
  height,
  left,
  top
...
const withRenderStateOverrides = command => regl => {
  ...
  const memoizedRender = memoize(props => {
    ...
    return regl(_objectSpread({}, reglCommand, {
      depth,
      blend: blend$$1
    }));
  ...
  const renderElement = props => {
    ...
    const blend$$1 = props.blend || ((_props$originalMarker2 = props.originalMarker) === null || _props$originalMarker2 === void 0 ? void 0 : _props$originalMarker2.blend) || reglCommand.blend || defaultReglBlend;
    memoizedRender({
      depth,
      blend: blend$$1
    })(props);
...
const cones = withRenderStateOverrides(fromGeometry(points$1, sideFaces$1.concat(endCapFaces$1)));
...
function Arrows(props) {
  ...
  }, passedProps), cylinders$$1), React__default.createElement(Cones, _extends({
    getChildrenForHitmap: getChildrenForHitmapWithOriginalMarker
  }, passedProps), cones$$1));
}
...
const lines = regl => {
  ...
  const memoizedRender = memoize(props => {
    const {
      ...
      blend: blend$$1 = defaultReglBlend
    } = props;
    return regl({
      depth,
      blend: blend$$1
...
const spheres = withRenderStateOverrides(fromGeometry(points$2, faces));
...
const createMemoizedDrawAtlasTexture = () => memoizeOne((textAtlas, atlasTexture) => {
  atlasTexture({
  ...
  const command = regl => {
    ...
    return (props, isHitmap) => {
      ...
      for (const marker of props) {
        ...
        const fgColor = toColor(isHitmap ? marker.color || [0, 0, 0, 1] : ((_marker$colors = marker.colors) === null || _marker$colors === void 0 ? void 0 : _marker$colors[0]) || marker.color || BG_COLOR_LIGHT$1);
        ...
        const bgColor = toColor(((_marker$colors3 = marker.colors) === null || _marker$colors3 === void 0 ? void 0 : _marker$colors3[1]) || (command.autoBackgroundColor && isColorDark(fgColor) ? BG_COLOR_LIGHT$1 : BG_COLOR_DARK$1));
...
export { Worldview, WorldviewBase as OffscreenWorldview, Bounds, camera, selectors as cameraStateSelectors, CameraStore, DEFAULT_CAMERA_STATE, CameraListener, eulerFromQuaternion, fromGeometry, parseGLB, WorldviewReactContext, pointToVec3, orientationToVec4, vec3ToPoint, vec4ToOrientation, pointToVec3Array, toRGBA, vec4ToRGBA, toColor, getCSSColor, defaultReglBlend, defaultReglDepth, defaultDepth, defaultBlend, blend, withPose, getVertexColors, colorBuffer, shouldConvert, intToRGB, getIdFromColor, getIdFromPixel, fromSpherical, Ray, getRayFromClick, Arrows$1 as Arrows, makeArrowsCommand, Axes, Command, SUPPORTED_MOUSE_EVENTS, Cones, cones, Cubes, cubes, Cylinders, cylinders, DrawPolygons, Polygon, PolygonPoint, PolygonBuilder, FilledPolygons, makeFilledPolygonsCommand, GLText, makeGLTextCommand, GLTFScene, Grid, Lines, lines, Overlay, Points, makePointsCommand, Spheres, spheres, Text, Triangles, makeTrianglesCommand, nonInstancedGetChildrenForHitmap, getChildrenForHitmapWithOriginalMarker, createInstancedGetChildrenForHitmap };
